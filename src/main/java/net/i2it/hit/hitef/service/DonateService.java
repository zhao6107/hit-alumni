...
import net.i2it.hit.hitef.service.function.JsSdkConfig;
...
public class DonateService {
    ...
    private Map<String, Object> getUnifiedOrderResult(String code, PrepayVO prepayVO) {
        ...
        if (webAccessTokenVO != null) {
            //获取统一下单的提交参数
            Map<String, Object> map = new UnifiedOrder().getUnifiedOrderInfo(webAccessTokenVO.getOpenid(), prepayVO);
            ...
            double totalFee = prepayVO.getMoney();//这个对象中的金额是以单位元记的
     ...
     */
    public Map<String, Object> getPayRequestInfo(String code, PrepayVO prepayVO) {
        Map<String, Object> map = null;
        if (prepayVO != null) {
            map = this.getUnifiedOrderResult(code, prepayVO);
        }
    ...
    public PrepayVO getPrepayVO(String itemInfo) {
        String[] arr = itemInfo.split("_");
        if (arr.length == 3) {
            return new PrepayVO(Integer.parseInt(arr[0]), arr[1], Double.parseDouble(arr[2]));
        }
        return null;
    }
